...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (userr?.avatar) {
    await deleteFromCloudinary(userr.avatar.public_id);
  }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  if (!coverLocalPath) throw new ApiError(400, "Cover image file is required");
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  //* so any channel you want to get is when we go into there profile url->req.params se data

  const { username } = req.params;
  if (!username?.trim()) throw new ApiError(400, "Channel id is required");

  //* now we need to find document of that user and with aggregate we will also get the subscribers of that user and number of subscribers he has done channels he has subscribed to
  ...
  const channel = await User.aggregate([
    ...
    //& second pipeline
    {
      $lookup: {
        ...
        localField: "_id", //user collection
        foreignField: "channel", //subscription collection
        as: "subscribers",
      },
    },
    //& third pipeline to get the number of channels the user has subscribed to
    {
      $lookup: {
        from: "Subscription",
        localField: "_id",
    ...
    //& fourth pipeline to addFlieds of counting both subscribers and channels
    {
      $addFields: {
        subscribersCount: {
          $size: "$subscribers",
    ...
    //& fifth pipeline to project the required fields
    {
      $project: {
        //? flag on-1 and off->0
        fullname: 1,
  ...
  console.log(channel); //todo optionally you get array and in our array we get one value only bz we are getting a users subs or chanel
